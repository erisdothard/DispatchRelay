...
export function scoreLoad(load: Load, prefs: CarrierPreferences): ScoredLoad {
  ...
  const equipmentPts =
    ...
    prefs.preferredEquipment.includes(load.equipment)
      ? 40
      : 0;
  ...
  const destMatch =
    prefs.preferredDestStates.length === 0 ||
  ...
  const daysUntilPickup =
    (new Date(load.pickupDate).getTime() - Date.now()) / 86_400_000;
