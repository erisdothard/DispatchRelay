...
export function BrokerCreditBadge({ companyId, inline = true }: BrokerCreditBadgeProps) {
  ...
  useEffect(() => {
    ...
    (supabase as any)
      ...
      .maybeSingle()
      .then(({ data }: { data: { avg_days_to_pay: number; on_time_pct: number; payment_count: number } | null }) => {
  ...
  const daysColor =
    metrics.avg_days_to_pay <= 14 ? 'text-green-400' :
    metrics.avg_days_to_pay <= 30 ? 'text-yellow-400' : 'text-red-400';

  const onTimeColor =
    metrics.on_time_pct >= 90 ? 'text-green-400' :
    metrics.on_time_pct >= 70 ? 'text-yellow-400' : 'text-red-400';
  ...
  return (
    <div className="rounded-xl p-3 space-y-2" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
      <p className="text-[10px] font-bold text-fx-text-muted uppercase tracking-widest">Broker Payment History</p>
      <div className="flex gap-4">
        <div>
          <div className={`text-lg font-black ${daysColor}`}>{Math.round(metrics.avg_days_to_pay)}d</div>
          <div className="text-[10px] text-fx-text-dim">avg days to pay</div>
        </div>
        <div>
          <div className={`text-lg font-black ${onTimeColor}`}>{Math.round(metrics.on_time_pct)}%</div>
