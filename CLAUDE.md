...
- **`/` and `/for/*`** — static HTML generated by `scripts/seo/generate.mjs` after the
...
- Phase 1 complete: static marketing site at `/` and `/for/{carriers,brokers,shippers}`;
  SPA moved to `/app`; waitlist endpoint; sitemap + robots; validators + 44 tests
- No pricing page by design — DispatchRelay is a custom build, not self-serve SaaS
